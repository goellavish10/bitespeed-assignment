...
const contactExists = (email, phoneNumber) => __awaiter(void 0, void 0, void 0, function* () {
    const query = `
    SELECT EXISTS (
      SELECT 1
      FROM contacts
...
const createPrimaryContactEntry = (email, phoneNumber) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const onlyPrimaryContacts = existingContacts.filter((contact) => contact.linkPrecedence === "primary");
    if (onlyPrimaryContacts.length >= 2) {
        onlyPrimaryContacts.splice(0, 1);
        const updateContactPromises = onlyPrimaryContacts.map((contact) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (existingContacts.length > 0) {
        ...
        if (!primaryContactChanged) {
            for (let i = 0; i < existingContacts.length; i++) {
                if ((existingContacts[i].email === email && phoneNumber === null) ||
                    (existingContacts[i].phoneNumber === phoneNumber && email === null)) {
                    createSecondaryContact = false;
                    break;
                }
                else if (existingContacts[i].email !== email ||
        ...
        if (createSecondaryContact) {
            const newContact = yield Contact_1.default.create({
                ...
                phoneNumber,
                linkPrecedence: "secondary",
                linkedId: existingContacts[0].linkPrecedence === "primary"
            ...
            if (existingContacts[0].linkPrecedence !== "primary") {
                const primaryContact = yield fetchPrimaryContact(existingContacts[0].linkedId);
                if (primaryContact) {
            ...
            const secondaryContactIdsArr = existingContacts.map((contact) => contact.linkPrecedence === "secondary" ? contact.id : null);
            return {
                primaryContactId: existingContacts[0].linkPrecedence === "primary"
                    ? existingContacts[0].id
                    : newContact.linkedId,
                emails: emails,
            ...
            if (existingContacts.length === 1 &&
                ...
                const secondaryContactsLeft = yield findAllRemainingSecondayContacts(existingContacts[0].id);
